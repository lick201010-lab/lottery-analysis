...
<template>
  <div class="space-y-8 animate-fade-in-up">
    <div>
      <h1 class="text-2xl font-bold text-white tracking-tight">模式分析</h1>
      <p class="text-base text-[#707a8a] mt-1">奇偶、大小、连号、区间与总和分布统计</p>
    </div>
    ...
    <template v-else-if="patternData">
      ...
      <div class="bg-[#1e2329] rounded-2xl border border-[#2b3139] p-6 shadow-sm card-lift">
        <div class="flex items-center gap-3 mb-5">
          ...
          <h3 class="text-base font-bold text-white">连号统计</h3>
        </div>
      ...
      <div v-if="patternData.range_distribution" class="bg-[#1e2329] rounded-2xl border border-[#2b3139] p-6 shadow-sm card-lift">
        <div class="flex items-center gap-3 mb-5">
          <div class="w-10 h-10 rounded-xl bg-[#2b3139] flex items-center justify-center">
          ...
          <h3 class="text-base font-bold text-white">区间分布</h3>
        </div>
      ...
      <div v-if="patternData.sum_distribution" class="bg-[#1e2329] rounded-2xl border border-[#2b3139] p-6 shadow-sm card-lift">
        <div class="flex items-center gap-3 mb-5">
          <div class="w-10 h-10 rounded-xl bg-[#2b3139] flex items-center justify-center">
          ...
          <h3 class="text-base font-bold text-white">总和分布</h3>
        </div>
        <div class="relative h-80">
      ...
      <div v-if="patternData.summary" class="bg-[#1e2329] rounded-2xl border border-[#2b3139] p-6 shadow-sm card-lift">
        <div class="flex items-center gap-3 mb-5">
          <div class="w-10 h-10 rounded-xl bg-[#2b3139] flex items-center justify-center">
            <svg class="w-5 h-5 text-[#707a8a]" fill="currentColor" viewBox="0 0 24 24"><path d="M19 3H5c-1.1 0-2 .9-2 2v14c0 1.1.9 2 2 2h14c1.1 0 2-.9 2-2V5c0-1.1-.9-2-2-2zM9 17H7v-7h2v7zm4 0h-2V7h2v10zm4 0h-2v-4h2v4z"/></svg>
          </div>
          <h3 class="text-base font-bold text-white">统计摘要</h3>
        </div>
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div v-for="(val, key) in patternData.summary" :key="key" class="p-5 bg-[#0b0e11] rounded-xl border border-[#2b3139]">
            <div class="text-xs font-bold text-[#707a8a] uppercase tracking-wider">{{ key }}</div>
            <div class="text-2xl font-extrabold text-white mt-2">{{ val }}</div>
          </div>
        </div>
      </div>
    </template>

    <div v-else class="bg-[#1e2329] rounded-2xl border border-[#2b3139] p-16 text-center shadow-sm">
      <svg class="w-12 h-12 mx-auto text-[#707a8a] mb-3" fill="currentColor" viewBox="0 0 24 24"><path d="M19 3H5c-1.1 0-2 .9-2 2v14c0 1.1.9 2 2 2h14c1.1 0 2-.9 2-2V5c0-1.1-.9-2-2-2zm-5 14H7v-2h7v2zm3-4H7v-2h10v2zm0-4H7V7h10v2z"/></svg>
      <p class="text-[#707a8a] text-lg">暂无模式分析数据</p>
    </div>
  </div>
</template>
