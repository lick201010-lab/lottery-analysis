...
<template>
  <div class="space-y-8 animate-fade-in-up">
    <div>
      <h1 class="text-2xl font-bold text-gray-900 tracking-tight">{{ lotteryLabel }} 号码生成</h1>
      <p class="text-base text-gray-400 mt-1">基于历史数据分析，智能生成号码组合</p>
    </div>
      ...
      <button
        ...
        :class="
          strategy === s.value
            ...
            : 'border-gray-100 bg-white hover:border-gray-200 hover:shadow-sm text-gray-600'
        "
      >
        <div class="flex items-center gap-3 mb-2">
          <div class="w-8 h-8 rounded-lg bg-gradient-to-br" :class="s.gradient" v-if="strategy === s.value"></div>
          <div class="w-8 h-8 rounded-lg bg-gray-100" v-else></div>
    ...
    <!-- Controls -->
    <div class="flex items-center gap-6 bg-white rounded-2xl border border-gray-200/80 p-6 shadow-sm">
      <div class="flex items-center gap-3">
        <label class="text-base font-semibold text-gray-600">生成组数</label>
        <div class="flex gap-1 bg-gray-50 p-1 rounded-xl">
          <button
            ...
            :class="count === n ? 'bg-white text-red-600 shadow-sm' : 'text-gray-400 hover:text-gray-600'"
          >
            {{ n }}
          </button>
        </div>
      ...
      <button
        @click="generate"
        :disabled="loading"
        class="btn-premium ml-auto inline-flex items-center gap-2 px-8 py-3.5 bg-gradient-to-r from-red-600 to-red-700 text-white text-base font-bold rounded-xl hover:shadow-lg hover:shadow-red-500/25 disabled:opacity-50"
      >
    ...
    <div v-if="result" class="space-y-5">
      <div
        ...
        :key="idx"
        class="bg-white rounded-2xl border border-gray-200/80 p-8 shadow-sm card-lift"
      >
        <div class="flex items-center gap-4 mb-6">
          <span class="text-2xl font-extrabold text-gray-200">#{{ idx + 1 }}</span>
          <span
        ...
        <div class="flex items-center gap-4 flex-wrap">
          <NumberBall
            ...
            size="xl"
          />
          <span class="text-3xl text-gray-200 mx-3 font-light">+</span>
        ...
        <div class="grid grid-cols-3 md:grid-cols-7 gap-3 mt-8">
          <div
            ...
            :key="'stat-' + n"
            class="text-center p-4 rounded-xl bg-gray-50 border border-gray-100 hover:border-gray-200 transition-colors"
          >
            ...
            <div v-else class="text-xs text-green-500 font-bold mt-1">最新出现</div>
          </div>
        </div>
      </div>
    </div>
    ...
    <div
      v-if="!result"
      class="bg-white rounded-2xl border border-gray-200/80 p-20 text-center shadow-sm"
    >
      <div class="w-20 h-20 rounded-2xl bg-gradient-to-br from-red-100 to-red-50 flex items-center justify-center mx-auto mb-6">
        <svg class="w-10 h-10 text-red-400" fill="currentColor" viewBox="0 0 24 24"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm0 18c-4.41 0-8-3.59-8-8s3.59-8 8-8 8 3.59 8 8-3.59 8-8 8zm3.5-9c.83 0 1.5-.67 1.5-1.5S16.33 8 15.5 8 14 8.67 14 9.5s.67 1.5 1.5 1.5zm-7 0c.83 0 1.5-.67 1.5-1.5S9.33 8 8.5 8 7 8.67 7 9.5 7.67 11 8.5 11zm3.5 6.5c2.33 0 4.31-1.46 5.11-3.5H6.89c.8 2.04 2.78 3.5 5.11 3.5z"/></svg>
      </div>
      <p class="text-xl font-bold text-gray-700">选择策略，点击「生成号码」</p>
      <p class="text-base text-gray-400 mt-2 max-w-md mx-auto">
