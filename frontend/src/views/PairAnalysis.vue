...
<template>
  <div class="space-y-8 animate-fade-in-up">
    <div>
      <h1 class="text-2xl font-bold text-[#0d253d] tracking-tight">数字组合分析</h1>
      <p class="text-base text-[#64748d] mt-1">探索数字之间的共现关系与最佳配对</p>
    </div>
    ...
    <div class="bg-white rounded-2xl border border-[#e3e8ee] p-6 shadow-sm card-stripe">
      <div class="flex items-center justify-between mb-5">
        <div class="flex items-center gap-3">
          ...
          <div>
            <h3 class="text-base font-bold text-[#0d253d]">数字选择器</h3>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-5">
          ...
          <table class="w-full table-premium text-[15px]">
            <thead class="sticky top-0 bg-white/95 backdrop-blur-sm z-10">
              <tr class="border-b border-[#e3e8ee]">
                <th class="px-5 py-3 text-left text-[#64748d] font-semibold">排名</th>
                <th class="px-5 py-3 text-left text-[#64748d] font-semibold">数字 A</th>
                <th class="px-5 py-3 text-left text-[#64748d] font-semibold">数字 B</th>
                <th class="px-5 py-3 text-right text-[#64748d] font-semibold">共现次数</th>
              </tr>
            </thead>
            <tbody>
              <tr
      ...
      <div v-if="selectedNumber" class="bg-white rounded-2xl border border-[#e3e8ee] p-6 shadow-sm card-stripe">
        <div class="flex items-center gap-3 mb-5">
          ...
          <h3 class="text-base font-bold text-[#0d253d]">配对分析概览</h3>
        </div>
        <div class="space-y-4">
          <div class="flex items-center gap-4 p-5 bg-[#f6f9fc] rounded-xl border border-[#e3e8ee]">
            ...
            <div>
              <div class="text-lg font-bold text-[#273951]">数字 #{{ selectedNumber }}</div>
              <div class="text-sm text-[#64748d]">
                共有 {{ pairDetails.length }} 个与之配对的数字记录
              </div>
            </div>
          </div>

          <div v-if="!loadingDetails && pairDetails.length > 0" class="mt-4">
            <div class="text-xs font-bold text-[#64748d] uppercase tracking-wider mb-3">最佳配对 Top 5</div>
            <div class="space-y-3">
              <div
      ...
      <div v-else class="bg-white rounded-2xl border border-[#e3e8ee] p-12 shadow-sm flex flex-col items-center justify-center text-center">
        ...
        <p class="text-[#64748d] font-medium text-lg">选择数字查看详细配对</p>
        <p class="text-[#64748d] text-sm mt-1">点击上方数字球或左侧表格中的数字</p>
      </div>
    </div>
  </div>
</template>
