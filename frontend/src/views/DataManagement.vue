...
<template>
  <div class="space-y-8 animate-fade-in-up">
    <div class="flex items-center justify-between">
      <div>
        <h1 class="text-2xl font-bold text-white tracking-tight">数据管理</h1>
        <p class="text-base text-[#707a8a] mt-1">查看历史开奖记录与数据更新</p>
      </div>
      <div class="flex items-center gap-4">
        <span class="text-base text-[#707a8a] font-medium">共 {{ total.toLocaleString() }} 条记录</span>
        <button
          @click="triggerScrape"
          :disabled="!!scrapeJobId"
          class="btn-premium inline-flex items-center gap-2 px-6 py-3 bg-[#fcd535] text-[#181a20] text-base font-bold rounded-xl hover:shadow-lg hover:shadow-red-500/25 disabled:opacity-50 disabled:cursor-not-allowed"
        >
    ...
    <div v-if="scrapeStatus" class="flex items-center gap-3 text-base text-blue-400 bg-blue-500/10 border border-blue-500/20 px-5 py-3 rounded-xl backdrop-blur-sm">
      ...
      <svg v-else class="w-5 h-5 text-blue-400" fill="currentColor" viewBox="0 0 24 24"><path d="M9 16.17L4.83 12l-1.42 1.41L9 19 21 7l-1.41-1.41z"/></svg>
      {{ scrapeStatus }}
    </div>
    ...
    <div v-if="total > perPage" class="flex items-center justify-center gap-3">
      <button
        ...
        :disabled="page <= 1"
        class="inline-flex items-center gap-1 px-5 py-2.5 text-base font-medium text-[#eaecef] border border-[#2b3139] rounded-xl disabled:opacity-30 hover:bg-[#2b3139] hover:shadow-sm transition-all bg-[#1e2329]"
      >
        <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M15.41 7.41L14 6l-6 6 6 6 1.41-1.41L10.83 12z"/></svg>
        上一页
      </button>
      <span class="text-base text-[#707a8a] font-medium px-4">
        第 {{ page }} / {{ totalPages }} 页
      </span>
      <button
        @click="changePage(page + 1)"
    ...
    <div class="bg-[#1e2329] rounded-2xl border border-[#2b3139] p-6 shadow-sm card-lift">
      <div class="flex items-center gap-3 mb-5">
        <div class="w-10 h-10 rounded-xl bg-[#2b3139] flex items-center justify-center">
          <svg class="w-5 h-5 text-[#707a8a]" fill="currentColor" viewBox="0 0 24 24"><path d="M19 3H5c-1.1 0-2 .9-2 2v14c0 1.1.9 2 2 2h14c1.1 0 2-.9 2-2V5c0-1.1-.9-2-2-2zm-5 14H7v-2h7v2zm3-4H7v-2h10v2zm0-4H7V7h10v2z"/></svg>
        </div>
        <h3 class="text-base font-bold text-white">爬取日志</h3>
      </div>
      <div v-if="scrapeLogs.length === 0" class="text-base text-[#707a8a] py-4">暂无日志</div>
      <div v-else class="overflow-x-auto">
        <table class="w-full table-premium text-[15px]">
          <thead>
            <tr class="text-left text-[#707a8a] border-b border-[#2b3139]">
          ...
          <tbody>
            <tr v-for="log in scrapeLogs" :key="log.id" class="text-[#b7bdc6] border-b border-[#2b3139]">
              ...
              <td class="py-3 pr-4">
                <span
                  ...
                  :class="log.status === 'success' ? 'bg-[#0ecb81]/10 text-[#0ecb81]' : log.status === 'failed' ? 'bg-[#f6465d]/10 text-[#f6465d]' : 'bg-blue-500/10 text-blue-400'"
                >
                  <span class="w-1.5 h-1.5 rounded-full" :class="log.status === 'success' ? 'bg-[#0ecb81]' : log.status === 'failed' ? 'bg-[#f6465d]' : 'bg-blue-500'"></span>
                  {{ log.status }}
                </span>
              </td>
