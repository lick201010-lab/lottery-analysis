...
useSEO({
  title: computed(() => t("双色球冷热号统计 & 六合彩号码频率分析")),
  description: computed(() =>
    t("实时统计号码在历史开奖中的出现频率、热号冷号排行、遗漏期数。数据仅供统计参考，不构成投注建议。"),
  ),
  lang,
  hreflangBase: "/frequency",
  hasEn: true,
});
...
const totalBallsLabel = computed(() =>
  lotteryType.value === "qxc" ? "0-14" : t("{n}球", { n: maxRegularNumber.value }),
);
...
<template>
  <div class="space-y-8 animate-fade-in-up">
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
      <div>
        <h1 class="text-2xl font-bold text-[#0d253d] tracking-tight">{{ t("号码统计") }}</h1>
        <p class="text-base text-[#64748d] mt-1">{{ t("数字出现频率、冷热分析与遗漏追踪") }}</p>
      </div>
    ...
    <template v-else>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-5 stagger-children">
        ...
        <div class="bg-white rounded-2xl border border-[#e3e8ee] p-6 shadow-sm card-stripe">
          <div class="flex items-center gap-3 mb-5">
            ...
            <h3 class="text-base font-bold text-[#0d253d]">{{ t("高频数字 (Top 10)") }}</h3>
          </div>
          <div v-if="hotColdData.hot.length === 0" class="text-base text-[#64748d] py-4">{{ t("暂无数据") }}</div>
          <div v-else class="space-y-2">
            ...
            >
              ...
              <span class="text-[15px] text-[#273951] flex-1 font-medium">
                {{ t("出现 {n} 次", { n: h.total_appearances }) }}
              </span>
        ...
        <div class="bg-white rounded-2xl border border-[#e3e8ee] p-6 shadow-sm card-stripe">
          <div class="flex items-center gap-3 mb-5">
            ...
            <h3 class="text-base font-bold text-[#0d253d]">{{ t("低频数字 (Bottom 10)") }}</h3>
          </div>
          <div v-if="hotColdData.cold.length === 0" class="text-base text-[#64748d] py-4">{{ t("暂无数据") }}</div>
          <div v-else class="space-y-2">
            <div
            ...
            >
              ...
              <span class="text-[15px] text-[#273951] flex-1 font-medium">
                {{ t("出现 {n} 次", { n: c.total_appearances }) }}
              </span>
              <span class="text-xs font-bold text-[#64748d] bg-[#f6f9fc] px-2 py-1 rounded-lg group-hover:bg-[#f6f9fc] group-hover:text-[#533afd] transition-colors">
                {{ t("评分 {n}", { n: c.hotness_score }) }}
              </span>
            </div>
          </div>
        </div>
      ...
      <div class="bg-white rounded-2xl border border-[#e3e8ee] p-6 shadow-sm card-stripe">
        <div class="flex items-center gap-3 mb-5">
          ...
          <h3 class="text-base font-bold text-[#0d253d]">{{ t("遗漏数字 (久未开出)") }}</h3>
        </div>
        <div v-if="overdueData.length === 0" class="text-base text-[#64748d] py-4">{{ t("暂无数据") }}</div>
        <div v-else class="grid grid-cols-3 sm:grid-cols-5 md:grid-cols-7 lg:grid-cols-8 gap-3">
          ...
          >
            ...
            <span class="text-xs font-semibold text-[#64748d]">{{ t("遗漏 {n} 期", { n: o.consecutive_missed }) }}</span>
          </div>
        </div>
      </div>
      ...
      <div class="bg-white rounded-2xl border border-[#e3e8ee] shadow-sm overflow-hidden card-stripe">
        <div class="px-6 py-5 border-b border-[#e3e8ee] flex items-center gap-3">
          ...
          <h3 class="text-base font-bold text-[#0d253d]">{{ t("完整排名 ({label})", { label: totalBallsLabel }) }}</h3>
        </div>
        <div class="overflow-x-auto">
          <table class="w-full table-premium text-[15px]">
            <thead>
              <tr class="bg-[#f6f9fc] border-b border-[#e3e8ee]">
                <th class="px-5 py-3.5 text-left text-[#64748d] font-semibold">{{ t("排名") }}</th>
                <th class="px-5 py-3.5 text-left text-[#64748d] font-semibold">{{ t("数字") }}</th>
                <th class="px-5 py-3.5 text-right text-[#64748d] font-semibold">{{ t("出现次数") }}</th>
                <th class="px-5 py-3.5 text-right text-[#64748d] font-semibold hidden sm:table-cell">
                  {{ lotteryType === 'ssq' ? t('蓝球次数') : t('特别号次数') }}
                </th>
                <th class="px-5 py-3.5 text-right text-[#64748d] font-semibold">{{ t("遗漏期数") }}</th>
                <th class="px-5 py-3.5 text-right text-[#64748d] font-semibold hidden lg:table-cell">{{ t("上次出现") }}</th>
                <th class="px-5 py-3.5 text-right text-[#64748d] font-semibold hidden md:table-cell">{{ t("热度") }}</th>
                <th class="px-5 py-3.5 text-center text-[#64748d] font-semibold">{{ t("趋势") }}</th>
              </tr>
            </thead>
            <tbody>
              <tr
              ...
              >
                ...
                <td class="px-5 py-3 text-center">
                  ...
                  >
                    {{ selectedNumber === r.number ? t('已选') : t('查看') }}
                  </button>
                </td>
              </tr>
            </tbody>
      ...
      <div v-if="selectedNumber" class="bg-white rounded-2xl border border-[#e3e8ee] p-6 shadow-sm card-stripe">
        <div class="flex items-center gap-3 mb-5">
          ...
          <div>
            <div class="text-base font-bold text-[#0d253d]">{{ t("数字 #{n} 近50期滚动频率", { n: selectedNumber }) }}</div>
            <div class="text-sm text-[#64748d]">{{ t("点击表格中其他数字可切换") }}</div>
          </div>
          <button
